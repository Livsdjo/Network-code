...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.pool1 = nn.AvgPool2d(kernel_size=4, stride=4, padding=0, ceil_mode=False, count_include_pad=True)
        self.pool2 = nn.AvgPool2d(kernel_size=4, stride=4, padding=0, ceil_mode=False, count_include_pad=True)
    ...
    def forward(self, x, y, xs_4, epoch):
        ...
        x_temp = self.pool1(x)
        x_temp = x_temp.reshape(2000, 128, -1)
        x_xulie = torch.cat([x_temp, center_feature1], dim=2)

        x = F.relu(self.bn7(self.conv7(x)))

        x_xulie = torch.cat([x_xulie, x.squeeze(3)], dim=2)
        """
        a0 = x.float()
        ...
        y_temp = self.pool1(y)
        y_temp = y_temp.reshape(2000, 128, -1)
        y_xulie = torch.cat([y_temp, center_feature2], dim=2)

        y = F.relu(self.bn7(self.conv7(y)))

        y_xulie = torch.cat([y_xulie, y.squeeze(3)], dim=2)
        """
        a0 = y.float()
        a2 = torch.norm(a0, p=2, dim=1)
        a2 = a2.unsqueeze(1)
...
class SA_Layer(nn.Module):
    ...
    def forward(self, image_f, point_f):
        ...
        x_q = self.q_conv(image_f[:, :, 5].unsqueeze(2)).permute(0, 2, 1) # b, n, c
        ...
        x_r = self.ContextNorm(self.trans_conv(x_r)) # point_f - x_r

        if 0:
            result = point_f * 0.5 + x_r * 0.5
        else:
            result = point_f * 0 + x_r
