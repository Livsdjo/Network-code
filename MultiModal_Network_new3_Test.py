...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(in_channels=2, out_channels=32, kernel_size=3, stride=1, padding=(1, 1), bias=True)
    ...
    def forward(self, x, y, xs_4, epoch):
        ...
        """
            下面这一块相当于Transformer
        """
        x = self.fusion(x, self.ContextNorm(point_out))
        # y = self.fusion(y, self.ContextNorm(point_out))
        ...
        if epoch <= 18:  # 3  10     18
            ...
            if 1:
                if self.start == 0:
                    fh = open('heheba_test原网络修改.txt', 'a', encoding='utf-8')
                    str1 = "分支开启"
                    fh.write(str1)
                    fh.close()
                    self.start = 1
        ...
        # OK了
        z = z.squeeze(2)
        z = self.ContextNorm(z) * 0.5 + self.ContextNorm(
            point_out) * 0.5  # self.ContextNorm(z) * 0 + self.ContextNorm(point_out)
        z = z.unsqueeze(2)
