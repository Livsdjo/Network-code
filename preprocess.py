...
def pre_handle(img, kpt_coeff, spec, num_corr, photaug, pert_homo, pert_affine, dense_desc, name):
    ...
    with tf.name_scope(name):  # pylint: disable=not-context-manager
        ...
        if dense_desc:
            ...
            # patch sampler.
            # print("11111")
            # print(img.shape, pert_kpt_affine.shape)
        ...
        return out, kpt_ncoords, pert_homo, patch
...
"""
def get_32_32_image():
    img0 = tf.compat.v1.placeholder(tf.float32, shape=(2, 2000, 2000, 1), name='img0')
    kpt_coeff0 = tf.compat.v1.placeholder(tf.float32, shape=(2, 2000, 6), name='kpt_coeff0')

    pert_homo = tf.numpy_function(
       get_rnd_homography, [2, 2, 0.15], tf.float32)
    pert_homo = tf.reshape(pert_homo, (2, 2, 3, 3))

    pert_affine = tf.numpy_function(
        get_rnd_affine, [2, 2, 2000], tf.float32)
    pert_affine = tf.reshape(pert_affine, (2, 2, 2000, 3, 3))
    ...
    if 0:
        net_input1, kpt_ncoords1, pert_homo1 = preprocess(
            img1, kpt_coeff1, None, num_corr, config['photaug'],
            pert_homo[1], pert_affine[1], config['dense_desc'], name='input1')
    
    f = tfpyth.torch_from_tensorflow(session, [img0, kpt_coeff0], [net_input0, kpt_ncoords0, pert_homo0]).apply
    print(326159683456)
    return f
"""

# img0, kpt_coeff0, img1, kpt_coeff1):
def get_32_32_image():
    if 0:
        print("aaaaaaaa")
        pert_homo = tf.numpy_function(
        ...
        with tf.device("GPU:1"):
            img0 = tf.placeholder(tf.float32, name="img0", shape=(1, None, None, 1))         # 1000  1000
            kpt_coeff0 = tf.placeholder(tf.float32, name="kpt_coeff0", shape=(1, 2000, 6))
            img1 = tf.placeholder(tf.float32, name="img1", shape=(1, None, None, 1))         # 1000  1000
            ...
            net_input0, kpt_ncoords0, pert_homo0, patch0 = pre_handle(
                img0, kpt_coeff0, None, 2000, True,
                pert_homo[0], pert_affine[0], False, name='input0')
            net_input1, kpt_ncoords1, pert_homo1, patch1 = pre_handle(
                img1, kpt_coeff1, None, 2000, True,
                pert_homo[1], pert_affine[1], False, name='input1')
        ...
        f3 = tfpyth.torch_from_tensorflow(session, [img0, kpt_coeff0], patch0).apply
        f4 = tfpyth.torch_from_tensorflow(session, [img1, kpt_coeff1], patch1).apply

        tf.get_default_graph().finalize()
        return f1, f2, f3, f4, session
