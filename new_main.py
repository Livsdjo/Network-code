...
def main(config, datasets, write=False, Image_Preprocess_XiaoRong=False, Use_Second_Network=False):
    ...
    if config.restore:
        ...
        va_res, val_loss = test_process("test", log_dir, '/NM-Net_888_best_state9_yanzheng_hao.pth', database, config, f1, f2,
                                       100000, datasets, config.knn_num)
    ...
    for epoch in range(config.epochs):
        loss_list = []
        for i, (img1s_z, img2s_z, img1s_z_g, img2s_z_g, xs_4, label, xs_12, index, others, xs_6) in enumerate(data, 0):
            if config.use_which_network == 0:
        ...
        # ----------------------------------------
        path_temp = '/NM-Net_888_state9_yanzheng_temp' + str(epoch) + '.pth'
        torch.save(Network, log_dir + path_temp)
        # torch.save(Network, log_dir + '/NM-Net_888_state9_yanzheng_temp.pth')
        # Validation
        va_res, val_loss = test_process("valid", log_dir, path_temp, database, config, f1, f2,
        ...
        if va_res > best_va_res:
            ...
            # Save best model
            if epoch <= 500:  # 15
                torch.save(Network, log_dir + '/NM-Net_888_best_state10_yanzheng_hao.pth')  # 1
            else:
                torch.save(Network, log_dir + '/NM-Net_888_best_state2_yanzheng_hao.pth')  # 2

    te_res, te_loss = test_process("test", log_dir, '/NM-Net_888_best_state10_yanzheng_hao.pth', database, config, f1,
                                   f2,  # 1
