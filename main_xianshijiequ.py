...
def main(config, datasets, write=False, Image_Preprocess_XiaoRong=False, Use_Second_Network=False):
    ...
    if config.restore:
        ...
        if 1:
            ...
            log_dir = "E:/NM-net-xiexie/hehba最终定型/结果3"
        else:
        ...
        if 0:
            ...
            save_file_cur = "E:/NM-net-xiexie/hehba最终定型/log"
        ...
        d = Data_Loader(config, database, database_list, "valid", initialize=False)  # train
    ...
    for epoch in range(config.epochs):
        ...
        for i, (img1s_z_g, img2s_z_g, xs_4, label, xs_12, index, others, xs_6) in enumerate(data, 0):
            if config.use_which_network == 0:
                ...
                if 1:
                    ...
                    kpts0 = np.stack([xs_4.cpu().numpy().squeeze(0).squeeze(0)[:, 0],
                                      xs_4.cpu().numpy().squeeze(0).squeeze(0)[:, 1]], axis=-1)
                    kpts1 = np.stack([xs_4.cpu().numpy().squeeze(0).squeeze(0)[:, 2],
                                      xs_4.cpu().numpy().squeeze(0).squeeze(0)[:, 3]], axis=-1)
                    ...
                    display, display1, display2, display3 = draw_matches(wanzheng_img0, wanzheng_img1, kpts0, kpts1,
                                                                         match_idx,
                                                                         [img0_1xy, img0_2xy, img0_3xy, img0_4xy],
                                                                         [img1_1xy, img1_2xy, img1_3xy, img1_4xy],
                                                                         label, downscale_ratio=1.0)  # 1.0
                ...
                if 0:
                    ...
                    patches0 = img1s.cpu().numpy().squeeze(1).reshape(2000, 32, 32)
                    patches1 = img2s.cpu().numpy().squeeze(1).reshape(2000, 32, 32)
                    match_n = 2000  # 100
        ...
        # ----------------------------------------
        torch.save(Network, log_dir + '/NM-Net_888_state.pth')
        # Validation
        va_res, val_loss = test_process("valid", log_dir, '/NM-Net_888_state.pth', database, config, f1, f2,
        ...
        if va_res > best_va_res:
            ...
            # Save best model
            if epoch <= 500:  # 15
                torch.save(Network, log_dir + '/NM-Net_888_best_state1.pth')
            else:
                torch.save(Network, log_dir + '/NM-Net_888_best_state2.pth')

    te_res, te_loss = test_process("test", log_dir, '/NM-Net_888_best_state1.pth', database, config, f1, f2,
